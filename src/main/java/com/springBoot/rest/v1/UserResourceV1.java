package com.springBoot.rest.v1;

import com.springBoot.dao.SpringDataDBUtils;
import com.springBoot.model.User;
//import com.springBoot.rest.HelloResource;
...
public class UserResourceV1 {
    ...
    public Response getUser(@ApiParam @PathParam("id") String id, @Context UriInfo uriInfo) {
        ...
        try {
            ...
            User u = this.get(id);
            LOGGER.info("User com.SpringBoot called with this id=" + id);
            if (u != null) {
    ...
    @GET
    @Path("v1/users")
    @ApiOperation(value = "Get an User resource. Version 1 - (version in URL)", response = User.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "User resource found"),
            @ApiResponse(code = 404, message = "User resource not found")
    })
    ...
    public Response createUser(User user, @Context UriInfo uriInfo) {
        ...
        try {
            ...
            if (validateErrors.isEmpty()) {
                LOGGER.debug("Calling user com.springBoot: " + user.getEmail());
                if (this.getByEmail(user.getEmail()) != null) {
                    apiResponse.put("error", "Email already Exist");
                    return Response.status(Response.Status.BAD_REQUEST).entity(apiResponse).build();
                }
                user = (User) this.create(user);
                if (user != null) {
                    LOGGER.debug("User Creation done with email :" + user.getEmail());
    ...
    * */
    public List<User> list(Integer offset, Integer limit) throws Exception {
        LOGGER.debug("Calling users service method ");
